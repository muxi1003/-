...
def main():
    ...
    for item in inputs:
        ...
        frame_count = float(item["frames"])
        if not frame_count.is_integer() or len(table)!=len(support) or len(table)!=int(frame_count) or not np.array_equal(support.frame_index,np.arange(len(table))):
