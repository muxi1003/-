...
def main():
    ...
    summary = dict(json_files=len(table), video_groups=table.video_group.nunique(),
        ...
        points_outside_nose_box_field_correction='includes missing same-group box associations; not a spatial-only count',
        points_without_same_group_nose_box=int((~pp.has_nose_box).sum()),
        points_geometrically_outside_available_same_group_box=int((pp.has_nose_box & ~pp.inside_nose_box).sum()),
