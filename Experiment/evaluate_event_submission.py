...
def main():
    ...
    parser.add_argument('--downloads', type=Path, default=Path('C:/Users/muxi/Desktop/标注'))
    ...
    sources = {'annotation_windows.csv': 'annotation_windows.csv', 'reference_events.csv': 'reference_events.csv',
    ...
    dependencies = [Path(__file__), REFERENCE / 'reference_tools.py', ROOT / 'experiment_common.py', HOLDOUT / 'score_external_counts.py',
                    HOLDOUT / 'run_frozen_external.py', HOLDOUT / 'frozen_rr_evaluation.py']
    code = []
    for i, path in enumerate(dependencies):
        snapshot = submission / 'code_snapshots' / f'{i:02d}_{path.name}'
        snapshot.parent.mkdir(exist_ok=True)
        shutil.copyfile(path, snapshot)
        code.append({'path': str(path), 'snapshot': str(snapshot), 'sha256': sha256(path)})
    write_json(submission / 'scoring_code_manifest.json', code)
    require(ref.TOLERANCE_SECONDS == .30, 'Protocol tolerance differs')
    write_json(submission / 'analysis_policy_before_scoring.json', {
        ...
        'submission_selection': '2026-09-17 user corrected source to Desktop/标注; earlier Downloads export superseded, do not mix outcomes',
